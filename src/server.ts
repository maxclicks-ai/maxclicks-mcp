...
const MCP_ENDPOINT = 'mcp.maxclicks.ai' as const;
...
app.use(
  cors({
    ...
    allowedHeaders: ['Content-Type', 'X-Maxclicks-API-Key', 'Authorization', 'MCP-Session-ID'],
  })
);
...
function getApiKeyFromRequest(req: Request): string | null {
  const pathMatch = req.path.match(/^\/([^\/]+)\/(v[12])\/mcp/);
  if (pathMatch) {
    return pathMatch[1];
  }
  return (
    (req.headers['x-maxclicks-api-key'] as string) ||
...
function createMcpServer(apiKey: string): McpServer {
  ...
  server.tool(
    ...
    async ({
      ...
      if (attributeValuesByKey !== undefined)
        updateData.attributeValuesByKey = attributeValuesByKey;
  ...
  server.tool(
    ...
    'Create a new custom attribute for contacts or objects. Use this to define custom fields.',
    {
      ...
      objectSchemaId: z
        .string()
        .optional()
        .describe('Required if targetType is "object" - the object schema ID'),
  ...
  server.tool(
    ...
    'Update an existing event schema. Use this to modify event schema details.',
    {
      ...
      payloadJsonSchema: z
        .record(z.any())
        .optional()
        .describe('Updated JSON schema for validation'),
    },
    async ({ schemaId, name, description, payloadJsonSchema }) => {
      ...
      Logger.metric('update-event-schema', duration, {
        success: true,
        schemaId,
      });
  ...
  server.tool(
    ...
    async ({ page, per_page }) => {
      const startTime = Date.now();
      Logger.info('Starting operation: list-object-schemas', {
        page,
        per_page,
      });
  ...
  server.tool(
    ...
    async ({ schemaSlug, id, objectId, attributeValuesByKey, tags, notes }) => {
      const startTime = Date.now();
      Logger.info('Starting operation: create-object', {
        schemaSlug,
        objectId,
      });
  ...
  server.tool(
    ...
    async ({ schemaSlug, objectId, attributeValuesByKey, tags, notes }) => {
      const startTime = Date.now();
      Logger.info('Starting operation: update-object', {
        schemaSlug,
        objectId,
      });
  ...
  server.tool(
    ...
    async ({ schemaSlug, objectId }) => {
      const startTime = Date.now();
      Logger.info('Starting operation: delete-object', {
        schemaSlug,
        objectId,
      });
...
function requireAuth(req: Request, res: Response, next: NextFunction) {
  ...
  if (!apiKey) {
    return res.status(401).json({
      error: 'Unauthorized',
      message:
        'API key required. Please provide X-Maxclicks-API-Key header or Authorization: Bearer <key>',
    });
  }

  (req as any).maxclicksApiKey = apiKey;
  next();
}

async function handleMcpRequest(req: Request, res: Response) {
  const apiKey = (req as any).maxclicksApiKey as string;
  ...
  try {
    ...
    res.on('close', () => {
      transport.close();
      const duration = Date.now() - startTime;
      Logger.debug('Connection closed', { method, requestId, duration });
    });
    ...
    const duration = Date.now() - startTime;
    Logger.metric('MCP request', duration, {
      method,
      requestId,
      success: true,
    });
    ...
    const duration = Date.now() - startTime;
    Logger.error('MCP request failed', error, { method, requestId, duration });
...
app.post('/:apiKey/v1/mcp', requireAuth, handleMcpRequest);
...
app.get('/', (req: Request, res: Response) => {
  res.json({
    name: 'Maxclicks MCP Server',
    version: '1.0.0',
    description: 'Model Context Protocol server for Maxclicks API',
    endpoints: {
      'MCP (Header Auth)': 'POST /mcp',
      'MCP (URL Auth v1)': 'POST /{api-key}/v1/mcp',
    ...
    authentication: {
      methods: [
        ...
        'URL path: /{api-key}/v1/mcp',
      ],
      getApiKey: 'https://app.maxclicks.ai/settings/developers',
    },
    documentation: 'http://docs.maxclicks.ai/mcp-server',
  });
});
...
app.listen(PORT, HOST, () => {
  ...
  const productionUrl = `https://${MCP_ENDPOINT}`;

  console.log('🚀 Maxclicks MCP Server');
  console.log(`🌐 Local Server: ${serverUrl}`);
  console.log(`🌍 Remote Server: ${productionUrl}`);
  console.log(`📡 MCP Endpoints:`);
  console.log(`• POST /mcp (header auth)`);
  console.log(`• POST /{api-key}/v1/mcp (URL auth)`);
  console.log(`❤️  Health: ${serverUrl + '/health'}`);
  console.log('🔐 Authentication Methods:');
  console.log('• X-Maxclicks-API-Key: {key}');
  console.log('• Authorization: Bearer {key}');
  console.log('• URL path: /{key}/v1/mcp');
  console.log('⚙️  Configuration:');
  console.log(`Retry Max Attempts: ${CONFIG.retry.maxAttempts.toString()}`);
  console.log(`Retry Initial Delay: ${CONFIG.retry.initialDelay.toString()}ms`);
  console.log(`Retry Max Delay: ${CONFIG.retry.maxDelay.toString()}ms`);
  console.log('\n📝 Local Testing (MCP tools/list):');
  console.log('   Header Auth:');
  console.log(`     curl -X POST ${serverUrl}/mcp \\`);
  console.log('       -H "X-Maxclicks-API-Key: YOUR_API_KEY" \\');
  console.log('       -H "Content-Type: application/json" \\');
  console.log('       -H "Accept: application/json, text/event-stream" \\');
  console.log('       -d \'{"jsonrpc":"2.0","id":1,"method":"tools/list"}\'');
  console.log('\n   URL Auth:');
  console.log(`     curl -X POST ${serverUrl}/YOUR_API_KEY/v1/mcp \\`);
  console.log('       -H "Content-Type: application/json" \\');
  console.log('       -H "Accept: application/json, text/event-stream" \\');
  console.log('       -d \'{"jsonrpc":"2.0","id":1,"method":"tools/list"}\'');
  console.log('\n📝 Production Endpoint:');
  console.log(`   ${productionUrl}/YOUR_API_KEY/v1/mcp`);
  console.log('\n✅ Server is ready to accept connections!');
  console.log('🔄 Using stateless mode - new transport per request');
});
