...
const conditionField = z
  .union([z.object({ type: z.literal('none') }), z.object({ type: z.literal('segment'), segmentId: z.string() })])
  .describe('Delivery condition. Defaults to { type: "none" }.')
...
export function registerWebhookTools(server: McpServer, client: Maxclicks): void {
  ...
  server.registerTool(
    'create_webhook',
    {
      ...
      inputSchema: {
        ...
        schemaId: z.string().nullable().optional().describe('Schema id or slug. Required for every trigger except "email event".'),
  ...
  server.registerTool(
    'update_webhook',
    {
      ...
      description: 'Update a webhook url, email event types, condition, and/or status. Trigger and schema are not updatable.',
      inputSchema: {
        id: idField,
