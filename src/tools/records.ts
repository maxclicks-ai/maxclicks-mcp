...
export function registerRecordTools(server: McpServer, client: Maxclicks): void {
  ...
  server.registerTool(
    ...
    args => runTool(async () => jsonResult(await client.records.create(args.schema, args.input as RecordInput)))
  )
  ...
  server.registerTool(
    ...
    args => runTool(async () => jsonResult(await client.records.upsert(args.schema, args.input as RecordInput)))
  )
  ...
  server.registerTool(
    ...
    {
      title: 'Update record',
      description: 'Partially update a record by id. Only provided keys change; an explicit null clears a nullable field.',
      inputSchema: { schema: schemaField, id: idField, input: recordInputField },
    },
    args => runTool(async () => jsonResult(await client.records.update(args.schema, args.id, args.input as RecordInput)))
  )
  ...
  server.registerTool(
    'get_contact_audit_trail',
    {
      ...
      description: 'List a contact consent and communication audit trail, newest first. Contact schemas only.',
      inputSchema: {
        schema: schemaField,
        id: idField,
    ...
    args =>
      runTool(async () =>
        pageResult(await client.records.auditTrail(args.schema, args.id, { channel: args.channel, limit: args.limit, offset: args.offset }))
      )
  )
}
