 ...
 */
export async function startHttp(options: { port: number; host: string; baseUrl?: string }): Promise<void> {
  const app = express()
  ...
  const handlePost = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!isInitializeRequest(req.body)) {
        res.status(400).json(rpcError('Missing session id; the first request must be an initialize request.'))
        return
      }
